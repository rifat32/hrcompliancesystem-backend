...
<body>
    ...
    <table style="margin-top:-30px">
        <tbody>
            ...
            <tr>
                <td class="file_title">Employees</td>
            </tr>
            <tr>
                <td>
        ...
        <tbody>
            @foreach ($users as $index => $user)
                <tr class="table_row">
                    ...
                    <td class="employee">
                        {{ $user->title . ' ' . $user->first_Name . ' ' . $user->last_Name . ' ' . $user->last_Name }}
                    </td>

                    <td class="email">{{ $user->email }}</td>
                    <td class="designation">{{ $user->designation->name ?? '' }}</td>
                    <td class="role">
                        {{ isset($user->roles[0]) && $user->roles[0]->name ? processString($user->roles[0]->name) : '' }}
                    </td>
