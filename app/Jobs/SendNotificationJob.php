...
class SendNotificationJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($this->type == "create") {
            $notification_description = (explode('_', $this->entity_name)[0]) . " taken for the user " . ($this->user->title . " " .$this->user->first_Name . " " . $this->user->middle_Name . " " . $this->user->last_Name);

        }
        if ($this->type == "update") {
            $notification_description = (explode('_', $this->entity_name)[0]) . " updated for the user " . ($this->user->title . " " .$this->user->first_Name . " " . $this->user->middle_Name . " " . $this->user->last_Name);

        }
        if ($this->type == "approve") {
            $notification_description = (explode('_', $this->entity_name)[0]) . " approved for the user " . ($this->user->title . " " .$this->user->first_Name . " " . $this->user->middle_Name . " " . $this->user->last_Name);

        }
        if ($this->type == "reject") {
            $notification_description = (explode('_', $this->entity_name)[0]) . " rejected for the user " . ($this->user->title . " " .$this->user->first_Name . " " . $this->user->middle_Name . " " . $this->user->last_Name);

        }
        if ($this->type == "delete") {
            $notification_description = (explode('_', $this->entity_name)[0]) . " deleted for the user " . ($this->user->title . " " .$this->user->first_Name . " " . $this->user->middle_Name . " " . $this->user->last_Name);
