...
{
    use ErrorUtil,  BasicUtil, ModuleUtil;
    /**
     *
     * @OA\Post(
     *      path="/v1.0/roles",
    ...
    public function getRoles(Request $request)
    {
        ...
        try {
            ...
            $rolesQuery = Role::with('permissions:name,id', "users")
                ...
                ->when(!(empty(auth()->user()->business_id)), function ($query) use ($request) {
                    return $query
                    ->whereNotIn("name",["business_owner#".auth()->user()->business_id])
                ...
                ->when(!empty($request->end_date), function ($query) use ($request) {
                 ...
                 $roles =  $this->retrieveData($rolesQuery, "id", "roles");
