...
class DashboardManagementControllerV2 extends Controller
{
    ...
    public function checkHoliday($date, $user_id)
    {
        ...
        $holiday = $this->workTimeManagementComponent->get_holiday_details($date, $user_id, $all_parent_department_ids);
 ...
 // Get all parent department IDs of the employee
            ...
            $holiday = $this->workTimeManagementComponent->get_holiday_details(today(), $employee->id, $all_parent_department_ids);
...
foreach ($expires_in_days as $expires_in_day) {
    ...
    public function presentAbsentHours(
        ...
        for ($date = $start_date->copy(); $date->lte($end_date); $date->addDay()) {
            ...
            $query = DB::table('employee_user_work_shift_histories')
                ...
                ->select(DB::raw('SUM(work_shift_detail_histories.schedule_hour * 3600) as total_seconds'))
 ...
 /**
     *
     * @OA\Get(
     *      path="/v1.0/business-manager-dashboard/present-absent-today",
     *      operationId="getBusinessManagerDashboardDataPresentAbsentToday",
     *      tags={"dashboard_management.business_manager"},
     *       security={
     *           {"bearerAuth": {}}
     *       },
     ...
     */
     ...
     public function getBusinessManagerDashboardDataPresentAbsentToday(Request $request)
     {
         ...
         try {
             ...
             $data["absent"] = $this->absentToday(
                 $all_manager_department_ids
             );

             $data["present"] = $this->presentToday(
                $all_manager_department_ids
            );
